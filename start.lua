...
function write_multi(x, y, angle, texts, font_name, font_size, color, font_slant, font_face, align, interline)
    ...
    for i = 1, #texts do
        ...
        if extents.height > max_height then
            max_height = extents.height
        end

        widths[i] = extents.width
    end
    ...
    for i = 1, #texts do
        xx = (max_width - widths[i]) * align - max_width / 2.0
        yy = max_height * interline * (i - 1) - max_height * interline * (#texts - 1.0) * 0.5 + max_height / 2.0

        cairo_translate(cr, xx, 0)
        cairo_translate(cr, 0, yy)
...
function start()
    ...
    infos[1][1][2] = get_raw_value("sysname")
    ...
    infos[1][2][1] = "Processor - " .. get_raw_value("freq_g cpu0") .. "GHz"
